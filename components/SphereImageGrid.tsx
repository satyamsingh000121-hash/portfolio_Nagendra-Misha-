...
export default function SphereImageGrid({
  ...
  const calculateWorldPositions = useCallback((): WorldPosition[] => {
    const positions: WorldPosition[] = imagePositions.map((pos, index) => {
      ...
      const distancePenalty = isPoleImage ? 0.3 : 0.6;
      ...
      const scale = centerScale * Math.max(0.6, 0.8 + depthScale * 0.4);
      ...
      >
        ...
        >
          <div className="relative aspect-square">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-3 right-3 w-8 h-8 bg-black/60 rounded-full text-white flex items-center justify-center hover:bg-black/80 transition-all cursor-pointer"
            >
              <X size={18} />
            </button>
          </div>
  ...
  if (!isMounted) {
    return (
      <div
        className="bg-stone-200/50 rounded-full animate-pulse flex items-center justify-center"
        style={{ width: containerSize, height: containerSize }}
      >
        <div className="text-stone-400 text-sm">Loading 3D Sphere...</div>
      </div>
    );
  }
